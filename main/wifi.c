...
WIFI_ERR_T wifi_init(void)
{
    ...
    // set log level to 'warning' to show all warnings and lower
    ...
    if (esp_err != ESP_OK)
    {
        console_windows_printf(CONSOLE_WINDOW_2, "esp_netif_init failed: %d\n", esp_err);
    ...
    if (esp_err != ESP_OK)
    {
        console_windows_printf(CONSOLE_WINDOW_2, "esp_event_loop_create_default failed: %d\n", esp_err);
        return WIFI_ERR_INIT_FAILED;
    }
    ...
    if (sta_netif == NULL)
    {
        console_windows_printf(CONSOLE_WINDOW_2, "esp_netif_create_default_wifi_sta failed: %d\n", esp_err);
        return WIFI_ERR_INIT_FAILED;
    }
    ...
    if (esp_err != ESP_OK)
    {
        console_windows_printf(CONSOLE_WINDOW_2, "esp_wifi_init failed: %d\n", esp_err);
        return WIFI_ERR_INIT_FAILED;
    }
    ...
    if (esp_err != ESP_OK)
    {
        console_windows_printf(CONSOLE_WINDOW_2, "esp_event_handler_register failed: %d\n", esp_err);
        return WIFI_ERR_INIT_FAILED;
    }
    ...
    if (esp_err != ESP_OK)
    {
        console_windows_printf(CONSOLE_WINDOW_2, "esp_wifi_set_storage failed: %d\n", esp_err);
        return WIFI_ERR_INIT_FAILED;
    }
    ...
    if (esp_err != ESP_OK)
    {
        console_windows_printf(CONSOLE_WINDOW_2, "esp_wifi_set_mode failed: %d\n", esp_err);
        return WIFI_ERR_INIT_FAILED;
    }
...
WIFI_ERR_T wifi_scan(void)
{
    ...
    if (esp_err != ESP_OK)
    {
        console_windows_printf(CONSOLE_WINDOW_2, "esp_wifi_scan_start failed: %d\n", esp_err);
        return WIFI_ERR_SCAN_FAILED;
    }

    uint16_t num_access_points;
    esp_err = esp_wifi_scan_get_ap_num(&num_access_points);
    if (esp_err != ESP_OK)
    {
        console_windows_printf(CONSOLE_WINDOW_2, "esp_wifi_scan_get_ap_num failed: %d\n", esp_err);
        return WIFI_ERR_SCAN_FAILED;
    }

    #define MAX_AP_RECORDS 10
    uint16_t num_ap_records = MAX_AP_RECORDS;
    static wifi_ap_record_t ap_records[MAX_AP_RECORDS];
    esp_err = esp_wifi_scan_get_ap_records(&num_ap_records, ap_records);
    if (esp_err != ESP_OK)
    {
        console_windows_printf(CONSOLE_WINDOW_2, "esp_wifi_scan_get_ap_records failed: %d\n", esp_err);
        return WIFI_ERR_SCAN_FAILED;
    }

    console_windows_printf(CONSOLE_WINDOW_2, "wifi_scan: %d networks found.\n", num_ap_records);
    if (num_ap_records > 0)
    {
...
WIFI_ERR_T wifi_connect(const char* ssid, const char* password, uint32_t timeout_msec)
{
    ...
    if (esp_err != ESP_OK)
    {
        console_windows_printf(CONSOLE_WINDOW_2, "esp_wifi_connect failed: %d\n", esp_err);
        return WIFI_ERR_CONNECT_FAILED;
    }
